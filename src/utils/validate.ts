...
// 单个字段多个rules验证
export async function fieldValid<T extends object>(fieldRule: FieldRule<T, keyof T>, formValue: T[keyof T], formData: T): Promise<FieldValidResult<T, keyof T>> {
...
// 单个rule验证
export async function ruleValid<T extends object>(fieldRule: FieldRule<T, keyof T>, rule: ValidRule<T, keyof T> = {}, val: T[keyof T], formData: T): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
        ...
        if (rule.type) {
            ...
            if (typeFilter instanceof RegExp) {
                if (!typeFilter.test(formValue)) return reject(rule.message ?? t('m.form.wrongFormat', {label: fieldRule.label ?? ''}))
            } else {
        ...
        if (rule.itemList) {
            ...
            if (item) {
                ...
                return reject(rule.message ?? t('m.form.valRestriction', {
