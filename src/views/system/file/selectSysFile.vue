  ...
  <div class="form-view">
    ...
    >
      <template #left-action>
        <el-button :disabled="selectRows.length===0 || selectRows.length > selectionLimit" type="primary"
                   @click="emit('select', selectRows)">选择
        </el-button>
      </template>
    </m-table>
  </div>
...
import { getFileById, postSaveFile, queryFileList } from '@/api/file/fileOperation'
...
// 表单字段根据表单数据变化，有所不同
const columns = ref([
  { prop: 'object', label: '对象存储key', disabled: true, width: 120, },
  { prop: 'name', label: '文件名', rules: { required: true } },
  { prop: 'contentType', label: '文件类型', disabled: true, comment: '文件的MIME类型', width: 100, },
  { prop: 'suffix', label: '文件后缀扩展名', disabled: true, width: 120, },
  { prop: 'size', label: '文件大小', disabled: true, formatter: (row, col, val) => filesize(val, { base: 2, standard: 'jedec' }), },
  ...
  { prop: 'imgRatio', label: '图片宽高比', disabled: true, width: 100, },
  ...
  { prop: 'createTime', label: '上传时间', disabled: true, width: 160 },
])
