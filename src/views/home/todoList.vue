<template>
  <m-table :columns="tableColumns" :data="data" layout="stretch" :is-page="false">
    <template #left-action>
      <el-text>待办列表</el-text>
      <el-link type="primary" style="margin-left: 10px" @click="more('点击了更多.')">更多</el-link>
    </template>
  </m-table>
</template>
    ...
    slots: {
      default: scope => {
        if (scope.row.title === '周报') {
          return <span>
            {scope.row.desc}
            <el-link type="primary" underline={false} onClick={() => more('周报已填写')}>填写</el-link>
          </span>
        }
