...
- 新增：^link(Table 表格) 组件新增 `persist-layout-key` 属性，指定项目全局唯一的key，用户可以持久化保存表格的列布局：包括列显隐，列宽，列顺序，列固定等设置
- 新增：^link(Form 表单) 组件新增 `newline` 属性，不管上一行是否有足够空间容纳，该表单项始终从新行开始
- 新增：^link(Form 表单) 组件新增 `colSize` 属性，用于手动指定单个表单项的最大宽度，用于动态计算一行可容纳表单项的数量
- 新增：^link(Table 表格) 组件新增支持内存分页排序
...
- 调整：^(不兼容调整)danger ^link(Table 表格) `is-sort-column` 已修改为 `show-setting`，需手动全局更改属性名
- 优化：代码生成编辑列时，当列属性名为空，填入字段名自动生成属性名
- 优化：代码生成器查询表名默认别名 `a`
- 优化：^link(Form 表单) 组件新增组件属性提示方便开发
- 修复：当菜单类型为 `iframe` 时，页签切换缓存失效的问题
- 修复：代码生成表单无法启用数据权限的问题
...
- 修复：^link(Table 表格) 组件 `列设置` 在序号列隐藏后样式错乱的问题
